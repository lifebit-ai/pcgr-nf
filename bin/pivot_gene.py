    ...
    for gene in df['SYMBOL'].unique():
        ...
        for column in columns:
            if column in PCGR_COLUMNS:
                row[column] = ",".join([str(x) for x in list(df[column][df['SYMBOL'] == gene].unique())])
            else:
                print("unrecognized column {}, skipping...".format(column))
        row['NUMBER_OF_VARIANTS'] = len(list(df['GENOMIC_CHANGE'][df['SYMBOL'] == gene].unique()))
