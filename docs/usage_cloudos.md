...
### 7.1 Provide PCGR data bundle

The PCGR data bundle is selected automatically by `pcgr-nf` based on the `genome` parameter. This behaviour can be overwritten by providing the data bundle directly through the `pcgr_data` parameter. This parameter accepts two types of input:

1. **A path to folder containing the PCGR data bundle for annotation**, listed on the [main page](https://github.com/lifebit-ai/pcgr-nf) of this repository. Both bundles for `grch37` and `grch38` can be provided in the same folder, but providing just the necessary bundle makes the execution of the workflow faster. It must follow this structure:
...
2. **A tarball file, compressed or not**. The data bundle, containing a single or both reference genomes, can be passed as a tarball file, with or without compression. The structure must still follow the example above. This option adds an additional time overhead in the execution of `pcgr-nf` for the expansion (and decompression) of the file.
